...
% read train protocol
trainfileID = fopen("D:\document\top\codes\End-to-End\Enter_End-to-End\LA\ASVspoof2019_LA_cm_protocols\ASVspoof2019.LA.cm.train.trn.txt");
trainprotocol = textscan(trainfileID, '%s%s%s%s%s');
fclose(trainfileID);
trainfilelist = trainprotocol{2};  
% read eval protocol
devfileID = fopen("D:\document\top\codes\End-to-End\Enter_End-to-End\LA\ASVspoof2019_LA_cm_protocols\ASVspoof2019.LA.cm.dev.trl.txt");
devprotocol = textscan(devfileID, '%s%s%s%s%s');
fclose(devfileID);
devfilelist = devprotocol{2}; 
% read eval protocol
evalfileID = fopen("D:\document\top\codes\End-to-End\Enter_End-to-End\LA\ASVspoof2019_LA_cm_protocols\ASVspoof2019.LA.cm.eval.trl.txt");
