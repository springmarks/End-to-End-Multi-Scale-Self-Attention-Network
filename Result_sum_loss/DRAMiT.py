...
class DRAMiTransformer(nn.Module):  # Reciprocal Attention Transformer Block
    def __init__(self, dim, num_head=4, chsa_head_ratio=0.2, window_size=2, shift=0, head_dim=None, qkv_bias=True, mv_ver=1,
...
if __name__ == '__main__':
    ...
    input = torch.randn(4, 1, 100, 750) # 输入B C H W
    conv=nn.Conv2d(1,64,kernel_size=1,stride=1,padding=0)
    input=conv(input)
    # input=input.reshape(input.size(0),input.size(1)*4,input.size(2)//4,input.size(3))
